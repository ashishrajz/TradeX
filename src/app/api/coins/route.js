...
async function cgFetch(url) {
  ...
  if (r.status === 429) throw new Error("CoinGecko error 429");
  if (r.status === 401) throw new Error("CoinGecko error 401");
  ...
  try {
    ...
    if (q) {
      ...
      if (q.includes(",")) {
        ...
        const s = await cgFetch(searchUrl);
        ids = s.coins.map((c) => c.id).slice(0, 10);
      }
      ...
      let mr = await cgFetch(marketsUrl);
      ...
      if (withChart) {
        const chartPromises = mr.map(async (coin) => {
          try {
            ...
            const h = await cgFetch(histUrl);
            return {
              ...coin,
              chart: h.prices.map(([t, p]) => ({ time: t, price: p })),
            };
          } catch {
            return coin;
          }
      ...
      await redis.set(cacheKey, JSON.stringify(mr), "EX", 600); // cache 10 min
      return new Response(JSON.stringify(mr), { status: 200 });
    }

    // --- Featured coins (default) ---
